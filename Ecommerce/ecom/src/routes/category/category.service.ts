...
import { DeleteParentCategoryException } from 'src/routes/category/category.error'
...
export class CategoryService {
  ...
  async delete({ id, deletedById }: { id: number; deletedById: number }) {
    try {
      ...
      if (category.parentCategoryId === null && children.length > 0) {
        throw DeleteParentCategoryException
      }
